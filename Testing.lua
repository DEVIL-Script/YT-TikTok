...
local fSetClipboard = setclipboard or toclipboard or function(text)
    warn("Clipboard function not available. Link:", text)
    return false
end
...
local function newPlatoBoost(identifierFunc, identifierType)
    ...
    local function copyLink()
        ...
        local clipboardSuccess, clipboardError = pcall(function()
            return fSetClipboard(linkOrError)
        end)
        
        if clipboardSuccess then
            return true, "Link copied to clipboard!"
        else
            -- If clipboard fails, still return success but with different message
            return true, "Link generated: " .. linkOrError
        end
    end
